...
import {
  IAccountLockedNotifyEvent,
  IPasswordChangedEvent,
  IResetPasswordRequestEvent,
} from '@app/shared';
...
export class AuthTemplateAdapter implements AuthNotificationTemplatePort {
  getAccountLockedMessage(payload: IAccountLockedNotifyEvent): INotificationRequest {
    ...
    const content = accountLockedHtmlTemplate.replace(
      /\{\{\s*lockedUntilMinutes\s*\}\}/g,
      String(lockedUntilMinutes),
    );
    return {
  ...
  getResetPasswordRequestMessage(payload: IResetPasswordRequestEvent): INotificationRequest {
    ...
    const content = resetPasswordHtmlTemplate.replace(
      /\{\{\s*(\w+)\s*\}\}/g,
