...
- **HTTP:** `POST /auth/verify-otp` (email, code) â€” validates OTP, marks email verified, publishes `user.verified`, returns JWT.
- **HTTP:** `POST /auth/signin` (email, password) â€” validates credentials and email verified; returns JWT. If the account is locked (after 3 failed attempts), returns `403` with a message to try again after X minutes. IP is captured from the request for audit. Rate limit per IP applies; when exceeded, returns `429 Too Many Requests` with optional `Retry-After` header.
