...
export type TRateLimitEndpointKey = 'signup' | 'verify-otp' | 'signin' | 'reset-password-link' | 'change-password';
...
export const rateLimitConfig = registerAs(
    ...
    (): IRateLimitConfig => ({
        signup: {
            ...
            windowSeconds: parseIntEnv('RATE_LIMIT_SIGNUP_WINDOW_SECONDS', defaultLimits.signup.windowSeconds),
        },
        'verify-otp': {
            max: parseIntEnv('RATE_LIMIT_VERIFY_OTP_MAX', defaultLimits['verify-otp'].max),
            windowSeconds: parseIntEnv('RATE_LIMIT_VERIFY_OTP_WINDOW_SECONDS', defaultLimits['verify-otp'].windowSeconds),
        },
        signin: {
            max: parseIntEnv('RATE_LIMIT_SIGNIN_MAX', defaultLimits.signin.max),
            windowSeconds: parseIntEnv('RATE_LIMIT_SIGNIN_WINDOW_SECONDS', defaultLimits.signin.windowSeconds),
        },
        'reset-password-link': {
            max: parseIntEnv('RATE_LIMIT_RESET_PASSWORD_LINK_MAX', defaultLimits['reset-password-link'].max),
            windowSeconds: parseIntEnv('RATE_LIMIT_RESET_PASSWORD_LINK_WINDOW_SECONDS', defaultLimits['reset-password-link'].windowSeconds),
        },
        'change-password': {
            max: parseIntEnv('RATE_LIMIT_CHANGE_PASSWORD_MAX', defaultLimits['change-password'].max),
            windowSeconds: parseIntEnv('RATE_LIMIT_CHANGE_PASSWORD_WINDOW_SECONDS', defaultLimits['change-password'].windowSeconds),
        },
    }),
);
