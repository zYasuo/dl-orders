...
describe('PaymentController', () => {
    ...
    describe('webhook', () => {
        it('returns 200 when payload is valid and signature is valid', async () => {
            const result = await controller.webhook({ type: 'payment', data: { id: 'mp-123' } }, 'ts=1,v1=abc', 'req-1');
        ...
        it('throws UnauthorizedException when signature is invalid', async () => {
            ...
            await expect(controller.webhook({ type: 'payment', data: { id: 'mp-123' } }, 'ts=1,v1=wrong', 'req-1')).rejects.toThrow(
        ...
        it('throws BadRequestException when type is payment but data.id is missing', async () => {
            await expect(controller.webhook({ type: 'payment', data: {} as { id: string } }, undefined, undefined)).rejects.toThrow(
                BadRequestException,
            );
            await expect(controller.webhook({ type: 'payment', data: {} as { id: string } }, undefined, undefined)).rejects.toThrow(
                'Webhook type payment requires data.id',
            );
        });

        it('accepts non-payment type without data.id and does not require signature validation', async () => {
            const result = await controller.webhook({ type: 'merchant_order', data: {} as { id: string } }, undefined, undefined);
            expect(result).toEqual({ received: true });
