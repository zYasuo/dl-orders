...
describe('FindAllProductsUseCase (integration)', () => {
    ...
    describe('execute', () => {
        ...
        it('returns all products from repository and caches them', async () => {
            const a = await productRepository.create({ name: 'Product A', description: 'Desc A', price: 10 });
            const b = await productRepository.create({ name: 'Product B', description: 'Desc B', price: 20 });
