...
export class ProductRepository extends IProductRepositoryPort {
    ...
    async create(params: ICreateProduct): Promise<ProductEntity | null> {
        ...
        return new ProductEntity(row.id, row.name, row.description!, row.price, row.createdAt, row.updatedAt);
    }

    async findById(id: string): Promise<ProductEntity | null> {
        const product = await this.db.product.findUnique({ where: { id } });
        if (!product) return null;
        return new ProductEntity(product.id, product.name, product.description!, product.price, product.createdAt, product.updatedAt);
    }
